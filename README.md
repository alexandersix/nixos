...
Home Manager installs Zed through its `programs.zed-editor` module. Neovim
remains the default handler for Markdown and JSON files as well as the terminal
`EDITOR` and `VISUAL`.

Zed's `settings.json` remains writable so settings changed through Zed's UI and
the theme generated by Noctalia continue to work. Declarative settings can be
added under `programs.zed-editor.userSettings`; Home Manager merges those values
